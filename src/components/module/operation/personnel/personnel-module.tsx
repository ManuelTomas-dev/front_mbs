...
import {
  PersonnelModuleSkeleton,
  PersonnelCardsSkeleton,
  PersonnelListSkeleton,
} from "./personnel-skeleton";
...
export function PersonnelModule() {
  ...
  const [selectedPersonnel, setSelectedPersonnel] = useState<IPersonnel | null>(
    null,
  );
  ...
  const [personnelToDelete, setPersonnelToDelete] = useState<IPersonnel | null>(
    null,
  );
  ...
  const ActionDropdown = ({ personnel }: { personnel: IPersonnel }) => (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end">
        ...
        <DropdownMenuItem
          onClick={() => handleAction("delete", personnel)}
  ...
  const filteredPersonnel = personnel.filter(
    (personnel) =>
      personnel.first_name.toLowerCase().includes(searchTerm.toLowerCase()) ||
      personnel.last_name.toLowerCase().includes(searchTerm.toLowerCase()),
  );

  const activePersonnel = filteredPersonnel.filter(
    (personnel) => personnel.is_active,
  ).length;
  const inactivePersonnel = filteredPersonnel.filter(
    (personnel) => !personnel.is_active,
  ).length;
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {filteredPersonnel.length > 0
                ? Math.round((activePersonnel / filteredPersonnel.length) * 100)
                : 0}
              %
            </div>
          </CardContent>
